...
class IncrementalTranscriber:
    ...
    def __init__(self, pubsub, whisper_mlx_model, num_mel_frames=500):
        ...
        self.whisper_mlx_model.encoder._positional_embedding = sinusoids(
            (num_mel_frames + 10) // 2, whisper_mlx_model.dims.n_audio_state).astype(mx.float16)

        # Maintain buffer of desired size
        # New audio will be appended to it and old audio will be dropped from the beginning
        # self.mel = mx.zeros((1, num_mel_frames, whisper_mlx_model.dims.n_mels), mx.float32)
        self.mel = mx.zeros((1, 0, whisper_mlx_model.dims.n_mels), mx.float32)
    ...
    async def transcribe(self, audio_bytes):
        ...
        # 16000 samples/second * 2 bytes/sample * 0.1 seconds
        max_bytes = 16000 * 2 * 0.1  
        self.audio_bytes_buffer = (self.audio_bytes_buffer + audio_bytes)[-int(max_bytes):]
        ...
        if not contains_speech:
            ...
            if self.mel.shape[1] > 0:
                result = self.transcribe_mel()
                # Reset mel without any trailing audio, as there is unlikely to be any words which span to the next chunk
                self.mel = mx.zeros((1, 0, self.whisper_mlx_model.dims.n_mels), mx.float32)
        ...
        new_mel = audio.log_mel_spectrogram(self.audio_arr, self.whisper_mlx_model.dims.n_mels)
        new_mel = new_mel.reshape(1, *new_mel.shape)
        ...
        if self.mel.shape[1] >= self.num_mel_frames - 100:
            result = self.transcribe_mel()
            # Keep the last 500ms of audio in the buffer in case it's needed to understand the next chunk
            self.mel = self.mel[:, -50:, :]
            if result.text.strip():
                await self.pubsub.publish(EventType.HEARD_SPEECH, result.text.strip())
                record_result(self.audio_arr, result.text)
            self.audio_arr = self.audio_arr[-8000:] # Last 500ms as 16khz

        # Otherwise keep accumulating and return empty result for now
        return
        
    def transcribe_mel(self, max_tokens=20):
        # Drop audio from beginning to ensure it fits in space
        if self.mel.shape[1] > self.num_mel_frames:
            self.mel = self.mel[:, -self.num_mel_frames:, :]
        ...
        input_mel = mx.concatenate([
                self.mel,
                mx.zeros((1, total_frames - self.mel.shape[1], self.whisper_mlx_model.dims.n_mels), dtype=mx.float32)
            ], axis=1)
